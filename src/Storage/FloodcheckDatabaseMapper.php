...
class FloodcheckDatabaseMapper
{
     ...
     * @throws \Exception
     */
    public function isFloodcheckExceeded(
        ...
        if ($this->countMatchingFloodcheckLogs(
                $floodcheck_log,
                $cutoff
            ) >= $max_attempts
        ) {
     ...
     * @throws \Exception
     */
    public function countMatchingFloodcheckLogs(
        FloodcheckLog $floodcheck_log,
        DateController $cutoff
    ): int {
     ...
     * @throws \Exception
     */
    public function saveFloodcheckLog(
        FloodcheckLog $floodcheck_log,
        DateController $cutoff,
        int $max_attempts
    ): int {
        ...
        if ($this->isFloodcheckExceeded(
            ...
            // Throw an exception
            throw new \Exception(
                'Maximum submissions have been exceeded for the allowed time.'
            );
    ...
    private function insertFloodcheckLog(FloodcheckLog $floodcheck_log): int
    {
        ...
        $request->getQueryController()
            ->insert(
                [
                    'created'    => $floodcheck_log->getCreated()
                        ->getDateAsString(),
